...
class TableScreen extends Screen {
  ...
  void displayDropdownMenu() {
      ...
      text("SHOW DEFAULT",  20 + 75, 130 + MENU_HEIGHT / 2); // Position the text inside the dropdown button
      
      // Draw dropdown button
      fill(240); // Light grey background for the dropdown menu button
      stroke(0); // Black border
      rect(175, 130, MENU_WIDTH, MENU_HEIGHT); // Rectangle for the dropdown button
      ...
      textAlign(CENTER, CENTER);
      text("CHOOSE ROWS",  175 + 75, 130 + MENU_HEIGHT / 2); // Position the text inside the dropdown button
      ...
      if (dropdownOpen) {
          for (int i = 0; i < menuOptions.length; i++) {
              ...
              stroke(0); // Black border
              rect(175, 130 + MENU_HEIGHT + i * ITEM_HEIGHT, MENU_WIDTH, ITEM_HEIGHT); // Rectangle for menu item
              ...
              textAlign(CENTER, CENTER);
              text(menuOptions[i], 175 + 75, 130 + MENU_HEIGHT + i * ITEM_HEIGHT + ITEM_HEIGHT / 2); // Position the text inside the menu item
  
              // Check if mouse is over menu item
              if (mouseX >= 175 && mouseX <= 175 + MENU_WIDTH && mouseY >= 130 + MENU_HEIGHT + i * ITEM_HEIGHT && mouseY <= 130 + MENU_HEIGHT + (i + 1) * ITEM_HEIGHT) {
                  if (mousePressed) {
      ...
      fill(240); // Light grey background for the dropdown menu button
      stroke(0); // Black border
      rect(330, 130, MENU_WIDTH, MENU_HEIGHT); // Rectangle for the dropdown button
      ...
      textAlign(CENTER, CENTER);
      text("CHOOSE COLUMNS",  330 + 75, 130 + MENU_HEIGHT / 2); // Position the text inside the dropdown button
      ...
      if (dropdown2Open) {
          for (int i = 0; i < columnNames.length; i++) {
              fill(240); // Light grey background for menu item
              stroke(0); // Black border
              rect(330, 130 + MENU_HEIGHT + i * ITEM_HEIGHT, MENU_WIDTH, ITEM_HEIGHT); // Rectangle for menu item
              ...
              textAlign(CENTER, CENTER);
              text(columnNames[i], 330 + 75, 130 + MENU_HEIGHT + i * ITEM_HEIGHT + ITEM_HEIGHT / 2); // Position the text inside the menu item
  
              // Check if mouse is over menu item
              if (mouseX >= 330 && mouseX <= 330 + MENU_WIDTH && mouseY >= 130 + MENU_HEIGHT + i * ITEM_HEIGHT && mouseY <= 130 + MENU_HEIGHT + (i + 1) * ITEM_HEIGHT) {
                  if (mousePressed) {
      ...
      fill(240); // Light grey background for the dropdown menu button
      stroke(0); // Black border
      rect(485, 130, MENU_WIDTH, MENU_HEIGHT); // Rectangle for the dropdown button
      ...
      textAlign(CENTER, CENTER);
      text("SELECT HIGHEST",  485 + 75, 130 + MENU_HEIGHT / 2); // Position the text inside the dropdown button
      
       if (dropdown3Open) {
          for (int i = 0; i < columnNames.length; i++) {
              fill(240); // Light grey background for menu item
              stroke(0); // Black border
              rect(485, 130 + MENU_HEIGHT + i * ITEM_HEIGHT, MENU_WIDTH, ITEM_HEIGHT); // Rectangle for menu item
              ...
              textAlign(CENTER, CENTER);
              text(columnNames[i], 485 + 75, 130 + MENU_HEIGHT + i * ITEM_HEIGHT + ITEM_HEIGHT / 2); // Position the text inside the menu item
  
              // Check if mouse is over menu item
              if (mouseX >= 485 && mouseX <= 485 + MENU_WIDTH && mouseY >= 130 + MENU_HEIGHT + i * ITEM_HEIGHT && mouseY <= 130 + MENU_HEIGHT + (i + 1) * ITEM_HEIGHT) {
                  if (mousePressed) {
                      selectedColumnIndex = i; // Set selected item index
                      onlyrows = false;
                      onlycolumns = false;
          ...
          fill(240); // Light grey background for the dropdown menu button
        stroke(0); // Black border
        rect(640, 130, MENU_WIDTH, MENU_HEIGHT); // Rectangle for the dropdown button
        ...
        textAlign(CENTER, CENTER);
        text("SELECT LOWEST",  640 + 75, 130 + MENU_HEIGHT / 2); // Position the text inside the dropdown button
        
         if (dropdown4Open) {
            for (int i = 0; i < columnNames.length; i++) {
                fill(240); // Light grey background for menu item
                stroke(0); // Black border
                rect(640, 130 + MENU_HEIGHT + i * ITEM_HEIGHT, MENU_WIDTH, ITEM_HEIGHT); // Rectangle for menu item
                ...
                textAlign(CENTER, CENTER);
                text(columnNames[i], 640 + 75, 130 + MENU_HEIGHT + i * ITEM_HEIGHT + ITEM_HEIGHT / 2); // Position the text inside the menu item
    
                // Check if mouse is over menu item
                if (mouseX >= 640 && mouseX <= 640 + MENU_WIDTH && mouseY >= 130 + MENU_HEIGHT + i * ITEM_HEIGHT && mouseY <= 130 + MENU_HEIGHT + (i + 1) * ITEM_HEIGHT) {
                    if (mousePressed) {
                        selectedColumnIndex = i; // Set selected item index
                        onlyrows = false;
                        onlycolumns = false;
    ...
    void draw() {
       ...
       if (onlycolumns == true){
        ...
        if (mouseX >= 175 && mouseX <= 175 + MENU_WIDTH && mouseY >= 130 && mouseY <= 130 + MENU_HEIGHT) {
            dropdownOpen = !dropdownOpen; // Toggle dropdown state
        } 
    
        // If the dropdown is open and the user clicks outside, close the dropdown
        if (dropdownOpen && (mouseX < 175 || mouseX > 175 + MENU_WIDTH || mouseY < 130 || mouseY > 130 + MENU_HEIGHT + menuOptions.length * ITEM_HEIGHT)) {
            dropdownOpen = false;
        }
        ...
        if (mouseX >= 330 && mouseX <= 330 + MENU_WIDTH && mouseY >= 130 && mouseY <= 130 + MENU_HEIGHT) {
            dropdown2Open = !dropdown2Open; // Toggle dropdown state
        } 
    
        // If the dropdown is open and the user clicks outside, close the dropdown
        if (dropdown2Open && (mouseX < 330 || mouseX > 330 + MENU_WIDTH || mouseY < 130 || mouseY > 130 + MENU_HEIGHT + columnNames.length * ITEM_HEIGHT)) {
            dropdown2Open = false;
        }
        ...
        if (mouseX >= 485 && mouseX <= 485 + MENU_WIDTH && mouseY >= 130 && mouseY <= 130 + MENU_HEIGHT) {
            dropdown3Open = !dropdown3Open; // Toggle dropdown state
            
        } 
    
        // If the dropdown is open and the user clicks outside, close the dropdown
        if (dropdown3Open && (mouseX < 485 || mouseX > 485 + MENU_WIDTH || mouseY < 130 || mouseY > 130 + MENU_HEIGHT + columnNames.length * ITEM_HEIGHT)) {
            dropdown3Open = false;
        }
        ...
        if (dropdown3Open) {
         ...
         if (mouseX >= 640 && mouseX <= 640 + MENU_WIDTH && mouseY >= 130 && mouseY <= 130 + MENU_HEIGHT) {
            dropdown4Open = !dropdown4Open; // Toggle dropdown state
            
        } 
    
        // If the dropdown is open and the user clicks outside, close the dropdown
        if (dropdown4Open && (mouseX < 640 || mouseX > 640 + MENU_WIDTH || mouseY < 130 || mouseY > 130 + MENU_HEIGHT + columnNames.length * ITEM_HEIGHT)) {
            dropdown4Open = false;
        }
...
void displayTable2() {
    ...
    for (int i = 0; i < numColumns; i++) {
        ...
        if(i==1) columnStartXPositions[1] += 25;
        float columnWidth = textWidth(data.table.getColumnTitle(i)) + 14;
        // Adjust xPosition for the next column based on current header width and additional space
        xPosition += textWidth(data.table.getColumnTitle(i)) + 15; // 90 includes padding for the box
...
void displayTable3Content(float[] columnStartXPositions) {
    ...
    String[] highestValueRowValues = highestValueLine.split(data.DELIMITER);
    for (int i = 0; i < numColumns; i++) {
...
void displayTable4Content(float[] columnStartXPositions) {
    ...
    // Display the highest value row
    String[] highestValueRowValues = lowestValueLine.split(data.DELIMITER);
    for (int i = 0; i < numColumns; i++) {
        // Pad each value to match the maximum width of its column
        String paddedValue = padValue(highestValueRowValues[i], columnWidths[i]);
        // Use the stored x position for alignment
